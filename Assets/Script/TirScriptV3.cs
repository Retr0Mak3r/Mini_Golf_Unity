...
public class TirScriptV3 : MonoBehaviour
{
    ...
    public void Shoot() // fonction de tir (déclenchée lors du clic souris)
    {
        ...
        {
            power = (int)slider.value * 500; // Nous récupérons la valueur du slider pour la puissance, puis *25 pr avoir une puiss suffisante
            ...
            forward.y = 0;
            balle.GetComponent<Rigidbody>().AddForce(forward * power); // on tire la balle
    ...
    private void Update()
    {
        ...
        if(balle.GetComponent<Rigidbody>().velocity.magnitude < 0.6f && slider.interactable == false)
        {
            arrow.enabled = true;
